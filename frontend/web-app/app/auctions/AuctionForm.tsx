...
export default function AuctionForm({auction} : Props) {
    ...
    const { control, handleSubmit, setFocus, reset,
        formState: { isSubmitting, isValid, isDirty, errors }, // Add errors here
        ...
        <form className={'flex flex-col mt-3'} onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className={'grid grid-cols-2 gap-3'}>
                <Input label={'Year'} name={'year'}
                       control={control} type={'number'} rules={{required: 'Year is required'}}/>
                <Input label={'Mileage'} name={'mileage'}
                       control={control} type={'number'} rules={{required: 'Mileage is required'}}/>
            </div>
            ...
            <>
                ...
                <div className={'grid grid-cols-2 gap-3'}>
                    ...
                    <DateInput label={'Auction end data/time'} name={'auctionEnd'}
                               control={control} dateFormat={'dd MMMM yyyy h:mm a'} showTimeSelect rules={{required: 'Auction end date is required'}}/>
                </div>
            </>
            }
            <div className={'flex justify-between'}>
                ...
                <Button
                    disabled={!isValid}
                    type={'submit'}
                    isProcessing={isSubmitting} outline color={'success'}>Submit</Button>
            </div>
        </form>
    );
}
